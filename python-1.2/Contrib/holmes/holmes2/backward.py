...
#
# optimizations:
# uses discrimination tree to select rules possibly matching
# a goal;  we keep the index with the kbase, instead of 
# recomputing it each time, here;  avoids scanning rule list 
# exhaustively for each goal (and calling match());  
#
# also uses an index tree to search for already-asked facts 
# (avoids an 'in' scan, with list equality at each leaf node);
#
# also eliminates backward.kbase dependency, by requiring
# kbase to be passed in to ask_user, report, input_yes;
# this is much cleaner than holmes' shared global variables;
####################################################################
